...
```datacode
# Загрузка данных
global data = read_file(path)                              # Базовое чтение
global data = read_file(path, "Sheet2")                    # Чтение конкретного листа XLSX
global data = read_file(path, 2)                           # Заголовок в строке 2 (0-based)
global data = read_file(path, 1, "DataSheet")              # Лист "DataSheet", заголовок в строке 1
...
- **CSV файлы** (.csv) - основной формат
  - Опциональный параметр `header_row` для указания строки заголовка (по умолчанию 0)
- **Excel файлы** (.xlsx) - поддерживается через `read_file()`
  - Опциональный параметр `sheet_name` для выбора конкретного листа
  - Опциональный параметр `header_row` для указания строки заголовка (по умолчанию 0)
- **Текстовые файлы** (.txt) - для простых данных

### Примеры использования `read_file`:

```datacode
# Базовое чтение (первая строка = заголовок, первый лист для XLSX)
data = read_file(path("data.csv"))

# Чтение XLSX с выбором листа
data = read_file(path("report.xlsx"), "Sales")

# Чтение CSV с заголовком в строке 2 (0-based индексация)
data = read_file(path("data.csv"), 2)

# Чтение XLSX с выбором листа и строки заголовка
data = read_file(path("report.xlsx"), 1, "DataSheet")  # Лист "DataSheet", заголовок в строке 1
```
